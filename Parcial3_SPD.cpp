//C++
//Cheimienst Hernandez Gomez
//SPD - Primer parcial - Parte 3

//------------------------------------
// Reemplazo valores por nombres específicos sin usar espacio de memoria
...
// Flags para cada estado. 1 sería "no presionado" y 0 sería "presionado"
int suma = 1;
int resta = 1;
...
void setup() {
  ...
  pinMode(F, OUTPUT);
  pinMode(G, OUTPUT);
  pinMode(SENSOR_FLEXION, INPUT);
  pinMode(SENSOR_LUZ, INPUT);  // Configura A3 como entrada analógica
  Serial.begin(9600);
}
...
void loop() {
  controlarLuz();  // Llamo a la función controlarLuz() para que entre en funcionamiento el sensor de luz
  ...
  {
    // Declaro la variable pulsador_presionado que guarda lo que me devuelva la función presionarPulsador()
  ...
  if (valorLectura)  // Si el interruptor retorna el valor "1"...
  {
    ...
    el funcionamiento de la función mostrarMultiplos5(contador) */
    timer++;  // Aumento el timer
    if (timer == 20)  // Si el timer llega al valor "20", entra el if...
    {
      timer = 0;  // El timer se reinicia
      contador++;  // Incrementa el valor del contador (actuando así de manera automática)
      mostrarMultiplos5(contador);  // Muestra en los visualizadores, el múltiplo de 5 más cercano al contador
...
//------------------------------------
//declaro una función para mostrar múltiplos de 5
void mostrarMultiplos5(int contador)
{
  // Apago los visualizadores
  prendeDigito(APAGADOS);

  // Obtengo el múltiplo de 5 más cercano al contador
  int multiplo5 = (contador / 5) * 5;

  // Muestro el múltiplo de 5 en la unidad
  mostrarNumero(multiplo5 % 10);
  prendeDigito(UNIDAD);

  // Muestro la decena como 0
  mostrarNumero(0);
  prendeDigito(DECENA);
}
...
void controlarLuz()
{
  lectura_luz = analogRead(SENSOR_LUZ);//guarda los valores analogicos del fotodiodo
