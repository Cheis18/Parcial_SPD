...
void mostrarAngulo()
{
  lectura = analogRead(SENSOR_FLEXION);//guarda los valores analogicos del sensor
  angulo = map(lectura, 236, 634, 0, 180);//configuro los valores analogicos minimos y maximos para crear un rango de 0 a 180.
  ...
  if(lectura>=309 && lectura<634)//39° a 180°
  {
    angulo=angulo+7;  
  }
  if(lectura==236)//0°
